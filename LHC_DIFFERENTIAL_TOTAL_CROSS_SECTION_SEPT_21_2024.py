...
def func1(x):
    return (
            K*(N/(4*pi))*
        ( (Nem**2+Nem_p**2)/x**2
         +(1/sqrt(2))*(Nem_p-Nem)*g*delta_q**2*(m**2/pi)*exp(-x/(4*m**2))/x
         +1/2*Nem_p*g**2*delta_q**2*kappa/mbar**2*(m**2/pi)**2*exp(-x/(2*mbar**2))
         +1/4*g**2*delta_q**4*(m**2/pi)**2*exp(-x/(2*m**2)) 
         +1/(4*sqrt(2))*g**3*delta_q**4*(kappa/mbar**2)*x*(m**2/pi)*exp(-x*((1/(4*m**2))+(1/(2*mbar**2)))) 
...
def func2(x):
    return (
            ...
            (  ( (Nem**2+Nem_p**2)/x**2 +(1/sqrt(2))*(Nem_p-Nem)*g*delta_q**2*(m**2/pi)*exp(-x/(4*m**2))/x 
                  + 1/2*Nem_p*g**2*delta_q**2*kappa/mbar**2*(m**2/pi)**2*exp(-x/(2*mbar**2))
                  + 1/(8*pi)*Nem*g**2*delta_q**4*m**2*(m**2/pi)**(.5)*exp(-x/(8*m**2))/x 
                  + 1/4*g**2*delta_q**4*(m**2/pi)**2*exp(-x/(2*m**2)) 
...
def term1(z):
    return (  (m**2/pi)*  (1/(1*sqrt(2))) * g *  delta_q_plot(z)**2 )
...
def sigmatotal_1bundle_em(z):
    return (
     K*N * ( (2*Nem_p/t_min) + ((m**2/pi)*(1/(1*sqrt(2)))* g * delta_q_plot(z)**2 )
        )
)
