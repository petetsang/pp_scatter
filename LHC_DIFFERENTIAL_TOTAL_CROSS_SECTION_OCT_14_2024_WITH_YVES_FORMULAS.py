...
def sigmatotal_1bundle(x): # total cross section with 1 bundle
    return ( K *N*(term1(x)) )

def sigmatotal_1bundle_em(z): #total cross section with 1 bundle and EM
    return (
...
def analytic_elastic_sigma_em(y):
    return(
        N*K/(4*pi)*(
            (Nem**2+Nem_p**2)/t_min
            # do not use interference terms between E&M and QCD, they are in different hilbert spaces
