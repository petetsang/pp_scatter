...
def func1(x):
    return (
            K*(N/(4*pi))*
        ( (Nem**2+Nem_p**2)/x+ (1/sqrt(2))*(Nem_p-Nem)*g*delta_q**2*exp(-x/(4*m**2))/x+1/2*Nem_p*g**2*delta_q**2*kappa/mbar**2*exp(-x/(2*mbar**2))
        +1/4*g**2*delta_q**4*exp(-x/(2*m**2)) + 1/(4*sqrt(2))*g**3*delta_q**4*(kappa/mbar**2)*x*exp(-x*((1/(4*m**2))+(1/(2*mbar**2)))) + 1/16*g**4*delta_q**4*(kappa**2/mbar**4)*x**2*exp(-x/(mbar**2))
            )
    )    
...
def func2(x):
    return (
            ...
            (  ( (Nem**2+Nem_p**2)/x**2+ (1/sqrt(2))*(Nem_p-Nem)*g*delta_q**2*exp(-x/(4*m**2))/x 
                  ...
                  +1/(8*pi)*Nem*g**2*delta_q**4*m**2*exp(-x/(8*m**2))/x + 1/4*g**2*delta_q**4*exp(-x/(2*m**2)) 
